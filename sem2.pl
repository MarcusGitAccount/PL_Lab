...
union([], [], Acc, R) :- R = Acc.
union([H | T], B,  Acc, R) :- (\+ member(H, Acc), union(T, B,  [H | Acc], R)); union(T,  B, Acc, R).
union([], [H | T], Acc, R) :- (\+ member(H, Acc), union([], T, [H | Acc], R)); union([], T, Acc, R).
union(A, B, R) :- union(A, B, [], R).

intersection([], _, Acc, R) :- R = Acc.
intersection([H | T], B, Acc, R) :- (member(H, B), intersection(T, B, [H | Acc], R)); intersection(T, B, Acc, R).
